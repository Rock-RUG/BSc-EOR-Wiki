...
window.addEventListener('pagehide',cancel);document.addEventListener('DOMContentSwitch',cancel);})();(function installHeaderSearchClose(){if(window.__mkHeaderSearchCloseBound)return;window.__mkHeaderSearchCloseBound=true;const style=document.createElement('style');style.textContent='.md-header .md-search__form > label.md-search__icon[for="__search"]{pointer-events:auto!important;z-index:2;cursor:pointer}';document.head.appendChild(style);window.addEventListener('click',event=>{const label=event.target?.closest?.('.md-header .md-search__form .md-search__icon[for="__search"]');const toggle=document.querySelector('input.md-toggle[data-md-toggle="search"], input#__search');if(!label||!toggle)return;const shell=label.closest('.md-search');const input=shell?.querySelector('input[data-md-component="search-query"]');const open=toggle.checked||shell?.classList?.contains('md-search--active')||document.documentElement?.classList?.contains('mk-hsf-open')||document.activeElement===input;if(!open){requestAnimationFrame(()=>{if(toggle.checked)label.closest('.md-search')?.querySelector('input[data-md-component="search-query"]')?.focus({preventScroll:true});});return;}
event.preventDefault();event.stopImmediatePropagation();window.__mkHeaderSearchCloseToken=(window.__mkHeaderSearchCloseToken||0)+1;toggle.checked=false;const active=document.activeElement;try{if(shell?.contains(active))active.blur();}catch(_){}
...
const searchLabel=cand.kind==='label'&&cand.forId==='__search';down={cand,x:e.clientX,y:e.clientY,ts:Date.now(),clickToken:lastClickToken,searchChecked:searchLabel?!!getSearchToggle()?.checked:null,searchCloseToken:searchLabel?Number(window.__mkHeaderSearchCloseToken||0):null};}catch(_){down=null;}},true);window.addEventListener('pointerup',(e)=>{try{if(!down)return;const now=Date.now();const dt=now-down.ts;const dx=Math.abs((e.clientX||0)-down.x);const dy=Math.abs((e.clientY||0)-down.y);const cand=down.cand;const clickTokenAtDown=down.clickToken;const searchCheckedAtDown=down.searchChecked;const searchCloseTokenAtDown=down.searchCloseToken;down=null;if(!cand||!cand.el)return;if(dt>TAP_MAX_TIME)return;if(dx>TAP_MAX_MOVE||dy>TAP_MAX_MOVE)return;setTimeout(()=>{try{if(lastClickToken!==clickTokenAtDown)return;if(searchCloseTokenAtDown!==null&&Number(window.__mkHeaderSearchCloseToken||0)!==searchCloseTokenAtDown)return;if(searchCheckedAtDown!==null){const toggle=getSearchToggle();if(toggle&&!!toggle.checked!==searchCheckedAtDown)return;}
if(cand.kind==='a'){const a=cand.el;const href=a.getAttribute('href')||'';const abs=toAbs(href);if(isHashOnly(href)){const id=href.replace(/^#/,'');if(!id)return;try{window.location.hash='#'+id;}catch(_){}
