...
const MASTERY_LS_KEY = "concept_mastery_v1";
  ...
  // ===== Dice UI for Continue random (concept pages) =====
  const LAST_FACE_KEY = "random_custom_last_dice_face_v1";
  const __crBannerDiceState = { timer: null, navigating: false };
...
function getSiteRootUrl() {
  ...
  // Inline SVG icons (runtime-safe, no MkDocs shortcodes)
  function crSvg(name, size) {
    ...
    if (name === "check-decagram-outline") {
      return `<svg ${common} ${stroke}><polygon points="12.0,3.0 17.3,4.7 20.6,9.2 20.6,14.8 17.3,19.3 12.0,21.0 6.7,19.3 3.4,14.8 3.4,9.2 6.7,4.7"/><path d="M8.5 12.5l2.2 2.2 4.8-5.2"/></svg>`;
    }
    ...
    if (name === "exit-to-app-outline") {
      return `<svg ${common} ${stroke}><path d="M14 4h4a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2h-4"/><path d="M10 16l4-4-4-4"/><path d="M14 12H4"/></svg>`;
    }
    if (name === "file-edit-outline") {
      return `<svg ${common} ${stroke}><path d="M14 3H8a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V9z"/><path d="M14 3v6h6"/><path d="M10 15l4.8-4.8a1.4 1.4 0 0 1 2 2L12 17H10z"/></svg>`;
    }
    return "";
  }

  function crSvgForLevel(m, size) {
    if (m === 3) return crSvg("shield-check-outline", size);
    if (m === 2) return crSvg("check-circle-outline", size);
    if (m === 1) return crSvg("help-circle-outline", size);
    if (m === 0) return crSvg("close-circle-outline", size);
    return "";
  }

  function crIcon(m, size) {
    return `<span class="cr-mi" style="display:inline-block;vertical-align:-0.18em;margin-right:2px;">${crSvgForLevel(m, size)}</span>`;
  }
  ...
  try {
    sessionStorage.setItem(CANDS_KEY, JSON.stringify(arr || []));
  } catch (_) {}
}
...
function isConceptPage(relPath) {
  ...
  "linear-gradient(135deg, rgba(63,81,181,.12), rgba(63,81,181,.05))";
    ...
    // inject tiny styles once
if (!document.getElementById("cr-mastery-style")) {
  const st = document.createElement("style");
  st.id = "cr-mastery-style";
  st.textContent = `
...
`;
    ...
    // Make the three action buttons share one calmer, smaller size.
    try {
      const refBtn = document.getElementById("cr-continue");
      const toolBtns = [document.getElementById("cr-view"), document.getElementById("cr-change")].filter(Boolean);
      const allBtns = [refBtn, ...toolBtns].filter(Boolean);
      allBtns.forEach((btn) => {
        ...
        btn.style.setProperty("max-width", "calc((100% - 36px) / 3)", "important");
        btn.style.setProperty("padding-top", "0.34rem", "important");
        btn.style.setProperty("padding-bottom", "0.34rem", "important");
        btn.style.setProperty("padding-left", "0.9rem", "important");
        btn.style.setProperty("padding-right", "0.9rem", "important");
        btn.style.setProperty("border-radius", "11px", "important");
        btn.style.setProperty("min-height", "2.28rem", "important");
        btn.style.setProperty("font-size", "0.65rem", "important");
        btn.style.setProperty("font-weight", "700", "important");
        btn.style.setProperty("line-height", "1.02", "important");
        btn.style.setProperty("letter-spacing", "0", "important");
        btn.style.setProperty("white-space", "nowrap", "important");
        btn.style.setProperty("vertical-align", "middle", "important");
        btn.style.setProperty("gap", "0.44rem", "important");
        btn.style.setProperty("box-shadow", "none", "important");
        btn.style.setProperty("filter", "none", "important");
        btn.style.setProperty("text-shadow", "none", "important");
        btn.style.setProperty("transform", "none", "important");
      });
      [document.querySelector("#cr-continue .cr-dice__ico"), document.querySelector("#cr-view .cr-tool__ico"), document.querySelector("#cr-change .cr-tool__ico")]
        .filter(Boolean)
        .forEach((ico) => {
          ico.style.setProperty("width", "auto", "important");
          ico.style.setProperty("height", "auto", "important");
          ico.style.setProperty("flex", "0 0 auto", "important");
          ico.style.setProperty("border-radius", "0", "important");
          ico.style.setProperty("border", "none", "important");
          ico.style.setProperty("background", "none", "important");
          ico.style.setProperty("box-shadow", "none", "important");
          ico.style.setProperty("padding", "0", "important");
          ico.style.setProperty("margin", "0", "important");
          const svg = ico.querySelector('svg');
          if (svg) {
            svg.style.setProperty("width", "0.82rem", "important");
            svg.style.setProperty("height", "0.82rem", "important");
            svg.style.setProperty("display", "block", "important");
          }
        });
    } catch (_) {}

    // Dice: init icon + desktop hover roll
    try {
      const btn = document.getElementById("cr-continue");
      if (btn && btn.dataset.crDiceBound !== "1") {
        btn.dataset.crDiceBound = "1";
        const ico = btn.querySelector(".cr-dice__ico");
        if (ico) setDiceFace(ico, lastFace);

        if (ico && isHoverPointer()) {
          btn.addEventListener("mouseenter", () => {
            if (__crBannerDiceState.navigating) return;
            beginHoverRoll(ico, __crBannerDiceState);
          });
          btn.addEventListener("mouseleave", () => {
            if (__crBannerDiceState.navigating) return;
            // Only visual, do NOT persist face unless user clicks.
            freezeToRandomFace(ico, __crBannerDiceState);
          });
        }
      }
    } catch (_) {}
    ...
    try {
      ...
      const lb = document.getElementById("cr-selftest-label");
      if (cb) cb.checked = on;
      if (lb && !crbHasShopItem(CRB_SELF_TEST_ITEM_ID)) lb.title = `Unlock ${CRB_SELF_TEST_ITEM_TITLE} · ${CRB_SELF_TEST_ITEM_PRICE} EORbits`;
      if (cb && !cb.__crbLockBound) {
        cb.__crbLockBound = true;
        cb.addEventListener("change", async () => {
          if (false && cb.checked && !crbHasShopItem(CRB_SELF_TEST_ITEM_ID)) {
            const ok = await crbOfferUnlock(CRB_SELF_TEST_ITEM_ID, CRB_SELF_TEST_ITEM_TITLE, CRB_SELF_TEST_ITEM_PRICE, "random-banner-self-test");
            if (!ok) { cb.checked = false; try { sessionStorage.removeItem(REVIEW_MODE_KEY); sessionStorage.removeItem(REVIEW_NAV_FLAG); } catch(_){} }
          }
        });
      }
    } catch (_) {}

const conceptId = normLoc(currentRelPath());

// ===== B3: only allow ONE rating per random-arrival =====
function getArrivalIdForThisPage() {
  try {
    const id = sessionStorage.getItem(ARRIVAL_ID_KEY) || "";
    const loc = normLoc(sessionStorage.getItem(ARRIVAL_LOC_KEY) || "");
    if (id && loc === conceptId) return id;
  } catch (_) {}
  return "";
}

// 如果不是随机到达（比如手动打开页面），也限制“本次页面加载只能选一次”
const arrivalId = getArrivalIdForThisPage();
const ratedKey = arrivalId ? ("cm_rated_arrival_v1:" + arrivalId) : ("cm_rated_page_v1:" + conceptId);

function alreadyRatedThisArrival() {
  try { return sessionStorage.getItem(ratedKey) === "1"; } catch (_) { return false; }
}

function markRatedThisArrival() {
  try { sessionStorage.setItem(ratedKey, "1"); } catch (_) {}
}

function disableMasteryButtons() {
  document.querySelectorAll("#custom-random-banner .cr-mastery-emoji").forEach((b) => {
    b.disabled = true;
...
// 如果已经评过分，刷新后也要保持禁用
if (alreadyRatedThisArrival()) disableMasteryButtons();

// ===== 2.5: bump "Seen" once per random arrival =====
try {
  if (arrivalId) {
    const viewKey = "cm_viewed_arrival_v1:" + arrivalId;
...
function getFirstPageTag() {
  // 1) meta[name="tags"] 或 meta[property="tags"]
  const meta =
  ...
  // 2) Material 的 tag 元素（常见是 .md-tag）
...
function resolveCourseFromTagOrFallback(conceptId) {
  const raw = getFirstPageTag().toLowerCase();
  const key = raw ? raw.split(/[-_]/)[0] : "";  // m1c-lecture01 -> m1c
  if (key && COURSE_TAG_MAP[key]) return COURSE_TAG_MAP[key];
  ...
  return conceptId.split("/").slice(-2, -1)[0] || "";
      ...
      const meta = {
        title: ((document.querySelector("h1") && document.querySelector("h1").textContent) || document.title || "").trim(),
        course: resolveCourseFromTagOrFallback(conceptId),
        coursePath: conceptId.split("/").slice(0, -1).join("/"),
      };

      // 优先用 ConceptMastery.bumpView（如果你已升级 concept-mastery.js）
      if (window.ConceptMastery && typeof window.ConceptMastery.bumpView === "function") {
        window.ConceptMastery.bumpView(conceptId, meta);
      } else {
        // fallback：直接写 localStorage（兼容你现在旧的 concept-mastery.js）
        const raw = localStorage.getItem("concept_mastery_v1");
        const all = raw ? JSON.parse(raw) : {};
        const rec = (all && all[conceptId] && typeof all[conceptId] === "object") ? all[conceptId] : {};
        rec.viewCount = (Number(rec.viewCount) || 0) + 1;
        // 顺便写入一些 meta（未来 manage mastery 表格会用到）
        if (!rec.title) rec.title = meta.title;
        if (!rec.course) rec.course = meta.course;
        if (!rec.coursePath) rec.coursePath = meta.coursePath;

        all[conceptId] = rec;
        localStorage.setItem("concept_mastery_v1", JSON.stringify(all));
      }

      // 让 banner 立刻刷新（你已经在监听 conceptMasteryChanged 了）
...
function syncMasteryUI() {
  const meta = document.getElementById("cr-mastery-meta");
  const btns = Array.from(document.querySelectorAll("#custom-random-banner .cr-mastery-emoji"));

  const rec = (window.ConceptMastery && typeof window.ConceptMastery.get === "function")
    ? window.ConceptMastery.get(conceptId)
    : null;

  // 关键：如果记录被删除了（rec 为空）或被设成 Unrated（m 为 null）
  // 就必须清掉 sessionStorage 的 ratedKey，并恢复可点击
  if (!rec || rec.m == null) {
    if (alreadyRatedThisArrival()) {
      try { sessionStorage.removeItem(ratedKey); } catch (_) {}
    }
    enableMasteryButtons();
  } else {
    // 有记录时，按 arrival/page 锁决定是否禁用
    if (alreadyRatedThisArrival()) disableMasteryButtons();
    else enableMasteryButtons();
  }

  const currentM = (rec && typeof rec.m === "number") ? rec.m : null;

  btns.forEach((b) => {
    const m = Number(b.getAttribute("data-m"));
    if (Number.isFinite(m) && currentM === m) b.classList.add("cr-mastery-selected");
    else b.classList.remove("cr-mastery-selected");
  });

  if (!meta) return;

  if (!rec || rec.m == null) {
    meta.textContent = "Not rated yet.";
    return;
  }

  // 计数显示（兼容 counts / reviewCount）
  let full = 0, know = 0, fuzzy = 0, dont = 0;
  if (rec.counts && typeof rec.counts === "object") {
    full = Number(rec.counts.full) || 0;
    know = Number(rec.counts.know) || 0;
    fuzzy = Number(rec.counts.fuzzy) || 0;
    dont = Number(rec.counts.dont) || 0;
  } else {
    const rc = Number(rec.reviewCount) || 0;
    if (currentM === 3) full = rc;
    else if (currentM === 2) know = rc;
    else if (currentM === 1) fuzzy = rc;
    else if (currentM === 0) dont = rc;
  }

  const rated = Number(rec.reviewCount) || (full + know + fuzzy + dont);
  const seen = Number(rec.viewCount) || 0;

  const dt = rec.lastReviewed ? new Date(rec.lastReviewed) : null;
  const when = dt ? dt.toLocaleString() : "";

  meta.innerHTML = `Seen: ${seen} · Rated: ${rated} (` +
    `${crIcon(3, 16)}${full} ${crIcon(2, 16)}${know} ${crIcon(1, 16)}${fuzzy} ${crIcon(0, 16)}${dont}` +
...
syncMasteryUI();
window.addEventListener("conceptMasteryChanged", syncMasteryUI);
if (!alreadyRatedThisArrival()) enableMasteryButtons();


document.querySelectorAll("#custom-random-banner .cr-mastery-emoji").forEach((btn) => {
  btn.addEventListener("click", () => {
    if (alreadyRatedThisArrival()) return;
markRatedThisArrival();
disableMasteryButtons();

    const m = Number(btn.getAttribute("data-m"));
    if (!window.ConceptMastery || !Number.isFinite(m)) return;

    window.ConceptMastery.set(conceptId, m);
    syncMasteryUI();

    // NOTE: do NOT remove "Mastered" from the session pool here.
    // The random picker already re-weights items (mastered becomes very unlikely).
  });
});

// ===== B4: Manage localStorage UI =====
const MASTERY_LS_KEY = "concept_mastery_v1";

function ensureMasteryModal() {
  if (document.getElementById("cm-mastery-modal")) return;

  const wrap = document.createElement("div");
  wrap.id = "cm-mastery-modal";
  wrap.style.cssText = `
  ...
  function toCourse(id) {
    ...
    parts.pop();
    const course = parts.length ? parts[parts.length - 1] : "";
    return course;
  }

  function toConcept(rec, id) {
  ...
  function render() {
    const all = (window.ConceptMastery && window.ConceptMastery._readAll) ? window.ConceptMastery._readAll() : {};
    const entries = Object.entries(all || {}).map(([id, raw]) => [id, window.ConceptMastery.get(id)]);

    // sort by course then concept
    entries.sort((a, b) => {
      const ca = toCourse(a[0]);
      const cb = toCourse(b[0]);
      if (ca !== cb) return ca.localeCompare(cb);
      return toConcept(a[1], a[0]).localeCompare(toConcept(b[1], b[0]));
    });

    const counts = { full: 0, know: 0, fuzzy: 0, dont: 0 };
    for (const [, rec] of entries) {
      const c = rec && rec.counts ? rec.counts : null;
      if (c) {
        counts.full += c.full || 0;
        counts.know += c.know || 0;
        counts.fuzzy += c.fuzzy || 0;
        counts.dont += c.dont || 0;
      }
    }

    document.getElementById("cm-summary").innerHTML =
      `Concepts: ${entries.length} · Ratings (` +
      `${crIcon(3, 16)}${counts.full} ${crIcon(2, 16)}${counts.know} ${crIcon(1, 16)}${counts.fuzzy} ${crIcon(0, 16)}${counts.dont}` +
      `)`;

    const tb = document.getElementById("cm-tbody");
    tb.innerHTML = entries.map(([id, rec]) => {
      const c = (rec && rec.counts) ? rec.counts : { full:0, know:0, fuzzy:0, dont:0 };
      const last = rec && rec.lastReviewed ? new Date(rec.lastReviewed).toLocaleString() : "";
      const cur = rec && typeof rec.m === "number" ? rec.m : "";
      return `
        <tr style="border-bottom:1px solid var(--md-default-fg-color--lightest);">
  ...
  // change current mastery without increment counts
  wrap.addEventListener("change", (e) => {
    const sel = e.target && e.target.classList && e.target.classList.contains("cm-level") ? e.target : null;
    if (!sel) return;
    ...
    window.ConceptMastery.setLevel(id, m, { course: toCourse(id) });
    render();
  });

  // delete one concept record
  wrap.addEventListener("click", (e) => {
    const btn = e.target && e.target.classList && e.target.classList.contains("cm-del") ? e.target : null;
    if (!btn) return;
    const id = btn.getAttribute("data-id");
    if (!id) return;
    if (!confirm("Delete this concept record?")) return;

    const all = window.ConceptMastery._readAll();
    delete all[id];
    window.ConceptMastery._writeAll(all);
    try { window.dispatchEvent(new CustomEvent("conceptMasteryChanged")); } catch (_) {}
    render();
  });

  document.getElementById("cm-clear").addEventListener("click", () => {
    if (!confirm("Clear all mastery data?")) return;
    try { localStorage.removeItem("concept_mastery_v1"); } catch (_) {}
    try { window.dispatchEvent(new CustomEvent("conceptMasteryChanged")); } catch (_) {}
    render();
  });
...
// 绑定按钮打开 modal
const manageBtn = document.getElementById("cr-mastery-manage");
if (manageBtn) {
...
document.getElementById("cr-continue").addEventListener("click", async (e) => {
  e.preventDefault();

  if (__crBannerDiceState.navigating) return;
  __crBannerDiceState.navigating = true;

  const contBtn = document.getElementById("cr-continue");
  const ico = contBtn ? contBtn.querySelector(".cr-dice__ico") : null;

  const currentId = normLoc(currentRelPath());

  // 每次点击都读最新 candidates
  let pool = readCandidates();

  // 关键：先把当前页移出池，避免抽到自己导致“看起来没反应”
  pool = pool.filter((x) => normLoc(x) !== currentId);

  if (window.ConceptMastery) {
    pool = window.ConceptMastery.filterActive(pool); // 过滤 Mastered
  }

  if (!pool.length) {
    __crBannerDiceState.navigating = false;
    alert("Random pool only contains this page (or all remaining pages are Mastered). Please edit filter.");
    return;
  }

  const chosen = (window.ConceptMastery && window.ConceptMastery.pickWeighted)
    ? window.ConceptMastery.pickWeighted(pool)
    : pickRandom(pool);
  ...
  // 写 arrival token：用于“本次到达只能评分一次”
  try {
    const arrivalId = newArrivalId();
    sessionStorage.setItem(ARRIVAL_ID_KEY, arrivalId);
    sessionStorage.setItem(ARRIVAL_LOC_KEY, normLoc(chosen));
  } catch (_) {}
  ...
  try {
    ...
    if (want) {
      if (!crbHasShopItem(CRB_SELF_TEST_ITEM_ID)) {
        const ok = await crbOfferUnlock(CRB_SELF_TEST_ITEM_ID, CRB_SELF_TEST_ITEM_TITLE, CRB_SELF_TEST_ITEM_PRICE, "random-banner-continue-self-test");
        if (!ok) {
          ...
          sessionStorage.setItem(REVIEW_NAV_FLAG, "1");
        }
      } else {
      sessionStorage.setItem(REVIEW_MODE_KEY, "1");
      sessionStorage.setItem(REVIEW_NAV_FLAG, "1");
  ...
  const targetUrl = toAbsoluteUrl(chosen);
  ...
  try {
    if (ico) {
      if (isHoverPointer()) face = freezeToRandomFace(ico, __crBannerDiceState);
...
document.getElementById("cr-change").addEventListener("click", (e) => {
  ...
  // 永远回到“开始 random 的入口页”（find 页面）
  let entry = "";
  try {
    entry = sessionStorage.getItem("random_custom_page_v1") || "";
  } catch (_) {}
  ...
  // 兜底：如果 entry 丢了，就回 find.html
  try {
    const root =
      (document.querySelector('script[src*="assets/javascripts/bundle"]') &&
        new URL(
          document.querySelector('script[src*="assets/javascripts/bundle"]').getAttribute("src"),
          document.baseURI
        )) ||
    ...
    const idx = p.indexOf("/assets/");
    const siteRoot = idx >= 0 ? root.origin + p.slice(0, idx + 1) : root.origin + (root.pathname.endsWith("/") ? root.pathname : root.pathname + "/");
    window.location.assign(new URL("find.html", siteRoot).toString());
  } catch (_) {
    window.location.assign("find.html");
  }
});
...
/* v27 injected icon colour hardening. */
(function(){
  try{
    if(document.getElementById('random-banner-icon-currentcolor-v27')) return;
    const st=document.createElement('style');
    st.id='random-banner-icon-currentcolor-v27';
...
/* v28: force random-session button icons to use light ink on dark UI. */
(function(){
  try{
    if(document.getElementById('random-banner-icon-ink-v28')) return;
    const st=document.createElement('style');
    st.id='random-banner-icon-ink-v28';
