...
(function () {
  ...
  const CR_SELF_TEST_ITEM_ID = "self_test_mode";
  const CR_SELF_TEST_ITEM_TITLE = "Self-test Mode";
  const CR_SELF_TEST_ITEM_PRICE = 0;

  function crShopApi() { try { return window.MkAccountData || null; } catch (_) { return null; } }
  function crHasShopItem(itemId) {
    try {
      if (itemId === CR_SELF_TEST_ITEM_ID) return true;
      const api = crShopApi();
  ...
  function crOfferUnlock(itemId, title, price, source) {
    try {
      if (crHasShopItem(itemId)) return Promise.resolve(true);
      const api = crShopApi();
      if (!api || typeof api.buyShopItem !== "function") {
        window.alert("The shop is still loading. Please try again in a moment.");
        return Promise.resolve(false);
      }
      const balance = crCurrencyBalance();
  ...
  function crEnsureSelfTestUnlocked(source) {
    return Promise.resolve(true);
  }
  function crInstallStylesOnce() {
    if (document.getElementById("mk-custom-random-shop-lock-style-v1")) return;
    const st = document.createElement("style");
    st.id = "mk-custom-random-shop-lock-style-v1";
    st.textContent = `
  ...
  // Remember last dice face so the next page can show the same icon.
  const LAST_FACE_KEY = "random_custom_last_dice_face_v1";

  function queueCustomRandomXp(detail) {
    try {
      const key = "mk_xp_pending_activity_queue_v1";
      const arr = JSON.parse(localStorage.getItem(key) || "[]");
      arr.push({ metric: "random_browse_start", details: detail || {}, opts: { scope: `random_browse_start:${detail && (detail.path || detail.eventName || Date.now())}`, throttleMs: 0 }, queuedAt: Date.now(), source: "custom-random-fallback" });
      localStorage.setItem(key, JSON.stringify(arr.slice(-300)));
    } catch (_) {}
  }

  function recordCustomRandomXp(detail) {
    try {
      const d = Object.assign({ source: "custom-random-page", path: location.pathname || "" }, detail || {});
      if (window.MkXpActivity && typeof window.MkXpActivity.record === "function") {
        window.MkXpActivity.record("random_browse_start", d);
        return;
      }
      if (window.MkXpActivity && typeof window.MkXpActivity.recordRandomBrowseStarted === "function") {
      ...
      queueCustomRandomXp(d);
      document.dispatchEvent(new CustomEvent("mk:xp-activity", { detail: Object.assign({ metric: "random_browse_start" }, d) }));
    } catch (_) {}
  }
  ...
  function isConceptLocation(loc) {
    ...
    // Exclude random*.html + custom-random
    if (base === "custom-random.html") return false;
    if (/^random[^\/]*\.html$/.test(base)) return false;

    // Exclude tool/meta pages.
    ...
    // Real concept pages live under Year-*/<course>/...
...
const expandState = state.expandState || {};
    ...
    const randomIco = randomBtn ? randomBtn.querySelector(".cr-dice__ico") : null;
    const randomDiceState = { timer: null, navigating: false };

    // PC: hover rolls the dice; mouseleave freezes immediately.
    if (randomBtn && randomIco && isHoverPointer()) {
      randomBtn.addEventListener("mouseenter", () => {
        if (randomDiceState.navigating) return;
        if (randomBtn.disabled) return;
        beginHoverRoll(randomIco, randomDiceState);
      });

      randomBtn.addEventListener("mouseleave", () => {
        if (randomDiceState.navigating) return;
        freezeToRandomFace(randomIco, randomDiceState);
      });
    }
    ...
    if (selfTestCb) {
      selfTestCb.addEventListener("change", async () => {
        if (selfTestCb.checked && !(await crEnsureSelfTestUnlocked("custom-random-self-test-toggle"))) {
          selfTestCb.checked = false;
        }
    ...
    // Start random：只从勾选池随机 (加上掌握度权重)
randomBtn.addEventListener("click", async (e) => {
  e.preventDefault();
  recordCustomRandomXp({ eventName: "custom-random-start-click" });
  if (randomDiceState && randomDiceState.navigating) return;
  ...
  let locs = poolDocs.map((r) => r.location);

  // 过滤掉“完全会了”
  if (window.ConceptMastery) {
    locs = window.ConceptMastery.filterActive(locs);
  }

  if (!locs.length) {
    alert("All selected pages are marked as Mastered. Please select more pages or change mastery.");
    return;
  }

  // 本次 random session 的池子存过滤后的
  storeCandidates(locs);

  // 加权抽取
  const chosen = window.ConceptMastery
    ? window.ConceptMastery.pickWeighted(locs)
    : pickRandom(locs);
  ...
  if (wantSelfTest && !(await crEnsureSelfTestUnlocked("custom-random-start-self-test"))) return;
  try {
    if (wantSelfTest) {
  ...
  if (randomIco) {
    if (isHoverPointer()) {
      face = freezeToRandomFace(randomIco, randomDiceState);
    } else {
      face = await rollDiceOnce(randomIco, { frames: 14, interval: 55 });
    }
    writeLastFace(face);
  }
