...
// ========== scroll unlock (single source of truth) ==========
// iOS Safari is sensitive to repeated unlock/scrollTo during interaction.
// Unlock once per page href and NEVER call scrollTo here.
let __mdUnlockHref = null;
function unlockMdScrollForThisPage() {
  const href = String(window.location && window.location.href || "");
  if (__mdUnlockHref === href) return;
  __mdUnlockHref = href;
...
function hardUnlockScroll() {
  ...
  /* OPTIMIZED: disabled forceCloseMaterialSearch (replaced by unlockMdScrollForThisPage) */
// 页面首次加载
// OPTIMIZED: removed forceCloseMaterialSearch DOMContentLoaded listener
// iOS bfcache / history restore
// OPTIMIZED: removed forceCloseMaterialSearch pageshow listener
  async function main() {
    ...
    // OPTIMIZED: single unlock (no scrollTo, no pageshow/touchstart listeners)
    unlockMdScrollForThisPage();
    ...
    rerender(container, state);
    // OPTIMIZED: do not remove .md-search__overlay (avoids breaking Material state)
  }
