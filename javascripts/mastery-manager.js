// docs/javascripts/mastery-manager.js
(function () {
  "use strict";
  ...
  const AIQ_KEY = "concept_quiz_sessions_v1";
  const STYLE_ID = "mm-manager-style-v17-cloud-sync";

  let mmSortKey = "recent";
  let mmSearchQuery = "";
  let mmRender = null;
  let mmLectureMap = null;
  let mmLectureMapPromise = null;
  let mmAiBank = null;
  let mmAiBankPromise = null;
  ...
  function mathToken(stash, latex) {
    const token = `@@MMATH${stash.length}@@`;
    const tex = String(latex || "").trim();
    stash.push({ token, html: tex ? `\\(${tex}\\)` : "" });
    return token;
  }

  function repairMathPollutedTitle(value) {
    const stash = [];
    let s = String(value || "");

    // Keep already-authored inline math intact while repairing polluted textContent
    // such as: Rn\mathbb R^nRn  ->  \(\mathbb{R}^{n}\).
    s = s.replace(/\\\((.+?)\\\)/g, (m) => {
      const token = `@@MMATH${stash.length}@@`;
      stash.push({ token, html: m });
      return token;
    });

    ["R", "C", "N", "Z", "Q"].forEach((sym) => {
      const bb = `\\\\mathbb\\s*\\{?\\s*${sym}\\s*\\}?`;

      // KaTeX/MathJax textContent often appears as plain-text left + TeX + plain-text right.
      // Examples: Rn\mathbb R^nRn, R2\mathbb{R}^{2}R2.
      const pollutedPow = new RegExp(`\\b${sym}\\s*([A-Za-z0-9]+)\\s*${bb}\\s*\\^\\s*\\{?\\s*([A-Za-z0-9]+)\\s*\\}?\\s*${sym}\\s*\\1\\b`, "g");
      s = s.replace(pollutedPow, (_, _plainExp, texExp) => mathToken(stash, `\\mathbb{${sym}}^{${texExp}}`));

      // Same problem, but without an exponent: R\mathbb RR.
      const pollutedBare = new RegExp(`\\b${sym}\\s*${bb}\\s*${sym}\\b`, "g");
      s = s.replace(pollutedBare, () => mathToken(stash, `\\mathbb{${sym}}`));

      // Raw unwrapped TeX accidentally stored in a title.
      const rawPow = new RegExp(`${bb}\\s*\\^\\s*\\{?\\s*([A-Za-z0-9]+)\\s*\\}?`, "g");
      s = s.replace(rawPow, (_, exp) => mathToken(stash, `\\mathbb{${sym}}^{${exp}}`));

      const rawBare = new RegExp(`${bb}`, "g");
      s = s.replace(rawBare, () => mathToken(stash, `\\mathbb{${sym}}`));
    });

    // Common asymptotic notation pollution from rendered math text.
    s = s
      .replace(/\b(Big|big)-O{2,}\b/g, (_, prefix) => `${prefix}-` + mathToken(stash, "O"))
      .replace(/\b(Little|little)-o{2,}\b/g, (_, prefix) => `${prefix}-` + mathToken(stash, "o"));

    // Single-letter inline math can also be polluted by textContent extraction.
    // Examples: p\(p\)p-test -> ppp-test, n\(n\)n-th Root -> nnn-th Root.
    // Repair only compact title patterns where the tripled letter is immediately
    // followed by a title separator, so normal words are not touched.
    s = s.replace(/\b([A-Za-z])\1{2}(?=-(?:test|th|axis|coordinate|norm|metric|root|lemma|case|value|variable|term|series|sequence|space|plane|line|method|rule|integral|derivative|function)\b)/gi, (_, letter) => mathToken(stash, letter));
    ...
    stash.forEach((item) => { s = s.split(item.token).join(item.html); });
    return s;
  }

  function cleanTitle(s) {
    return repairMathPollutedTitle(s);
  }
  ...
  function prettyCourseName(raw) {
    ...
    if (!s) return "";
    const maybeKey = s.toLowerCase().split(/[-_]/)[0];
    if (COURSE_TAG_MAP[maybeKey]) return COURSE_TAG_MAP[maybeKey];
    const t = s.toLowerCase();
    if (t.includes("math-i-calculus")) return COURSE_TAG_MAP.m1c;
    if (t.includes("math-ii-linear-algebra")) return COURSE_TAG_MAP.m2la;
    if (t.includes("math-iii-analysis")) return COURSE_TAG_MAP.m3a;
    if (t.includes("multivariate-calculus")) return COURSE_TAG_MAP.m4mc;
    if (t.includes("advanced-linear-algebra")) return COURSE_TAG_MAP.m5ala;
    if (t.includes("convexity") || t.includes("optimization")) return COURSE_TAG_MAP.m6co;
    if (t.includes("or-modelling") || t.includes("orm")) return COURSE_TAG_MAP.orm;
    return s.replace(/[-_]+/g, " ").replace(/\b\w/g, (m) => m.toUpperCase());
  }

  function deriveCourseName(loc) {
  ...
  function deriveLectureLabel(loc) {
    const segs = String(loc || "").split("/").filter(Boolean);
    const candidates = segs.slice(0, -1);
    for (let i = candidates.length - 1; i >= 0; i -= 1) {
      const m = String(candidates[i] || "").match(/^(lecture|week)[-_ ]*0*(\d+)$/i);
      if (m) return `${unitNounFromType(m[1])} ${Number(m[2])}`;
    }
    return "";
  }
  ...
  function hrefForLoc(loc) {
    try { return new URL(String(loc || ""), getSiteRootUrl()).href; }
  ...
  function readAll() {
    try {
      if (window.MkAccountData && typeof window.MkAccountData.getMasteryMap === "function") {
        const map = window.MkAccountData.getMasteryMap();
  ...
  function writeQuizSessions(obj) {
    try { localStorage.setItem(AIQ_KEY, JSON.stringify(obj || {})); } catch (_) {}
    try { window.dispatchEvent(new CustomEvent("mk-ai-quiz-sessions-changed", { detail: { source: "mastery-manager" } })); } catch (_) {}
  }

  function validLevel(m) {
    return [0, 1, 2, 3].includes(Number(m));
  }

  function historyKind(h) {
    const kind = String(h && (h.kind || h.type || h.event || h.action) || "").toLowerCase().trim();
    if (kind === "view" || kind === "visit" || kind === "seen") return "view";
    return "mastery";
  }

  function historyTs(h) {
    return Number(h && (h.ts || h.time || h.at || h.date)) || 0;
  }

  function isAiSource(source) {
    return /(?:ai|mcq|quiz)/i.test(String(source || ""));
  }

  function normaliseHistoryFallback(history, currentLevel, lastReviewed, reviewCount, lastViewed) {
    const src = Array.isArray(history) ? history : [];
    const out = [];
    for (const item of src) {
      const ts = historyTs(item);
      if (!ts) continue;
      const srcName = typeof item?.source === "string" ? item.source : "";
      if (historyKind(item) === "view") {
        out.push({ kind: "view", ts, source: srcName, visitId: item && item.visitId || "" });
        continue;
      }
      const mm = Number(item && (item.m ?? item.level ?? item.mastery));
      if (!validLevel(mm)) continue;
      out.push({ kind: "mastery", m: mm, ts, source: srcName });
    }
    ...
    if (Number(lastViewed) > 0 && !out.some((x) => x.kind === "view" && Number(x.ts) === Number(lastViewed))) {
      out.push({ kind: "view", ts: Number(lastViewed), source: "legacy", visitId: "" });
    }
    out.sort((a, b) => (Number(a.ts) || 0) - (Number(b.ts) || 0));
    return out;
  }
  ...
  function normaliseRecord(rec) {
    ...
    const counts0 = r.counts && typeof r.counts === "object" ? r.counts : {};
    const counts = explicitUnrated ? { full: 0, know: 0, fuzzy: 0, dont: 0 } : {
      full: Number(counts0.full) || 0,
      know: Number(counts0.know) || 0,
      fuzzy: Number(counts0.fuzzy) || 0,
      dont: Number(counts0.dont) || 0,
    };
    ...
    let history = normaliseHistoryFallback(r.history || r.reviewHistory || r.masteryHistory, m, lastReviewed, reviewCount, lastViewed);
    if (explicitUnrated) history = history.filter((h) => historyKind(h) === "view");
    const historyViews = history.filter((h) => historyKind(h) === "view").length;
    const viewCount = Math.max(Number(r.viewCount) || 0, reviewCount, historyViews);
    const visitCount = Math.max(Number(r.visitCount) || 0, viewCount, reviewCount);
    return {
      m,
      ...
      visitCount,
      lastViewed,
      visited: !!(r.visited || visitCount > 0 || lastViewed > 0),
      counts,
      history,
  ...
  function mClass(m) {
    if (m === 3) return "mm-level-mastered";
    if (m === 2) return "mm-level-clear";
    if (m === 1) return "mm-level-fuzzy";
    if (m === 0) return "mm-level-unknown";
    return "mm-level-unrated";
  }

  function mmSvg(name, size) {
    const s = Number(size) || 18;
    const common = `xmlns="http://www.w3.org/2000/svg" class="mm-svg-icon" width="${s}" height="${s}" viewBox="0 0 24 24" aria-hidden="true" focusable="false"`;
    ...
    if (name === "eye-outline") return `<svg ${common} ${stroke}><path d="M2 12s3.5-6 10-6 10 6 10 6-3.5 6-10 6-10-6-10-6z"/><circle cx="12" cy="12" r="2.5"/></svg>`;
    if (name === "quiz") return `<svg ${common} ${stroke}><path d="M9 3h6"/><path d="M10 3v4.5L5.5 16a3.6 3.6 0 0 0 3.2 5h6.6a3.6 3.6 0 0 0 3.2-5L14 7.5V3"/><path d="M8 14h8"/><path d="M9.5 17h5"/></svg>`;
    if (name === "delete") return `<svg ${common} ${stroke}><path d="M4 7h16"/><path d="M10 11v6M14 11v6"/><path d="M6 7l1 14h10l1-14"/><path d="M9 7V4h6v3"/></svg>`;
    if (name === "download") return `<svg ${common} ${stroke}><path d="M12 3v12"/><path d="M7 10l5 5 5-5"/><path d="M5 21h14"/></svg>`;
    if (name === "upload") return `<svg ${common} ${stroke}><path d="M12 21V9"/><path d="M7 14l5-5 5 5"/><path d="M5 3h14"/></svg>`;
    return "";
  }

  function mmSvgForLevel(m, size) {
    if (m === 3) return mmSvg("shield-check-outline", size);
    if (m === 2) return mmSvg("check-circle-outline", size);
    if (m === 1) return mmSvg("help-circle-outline", size);
    if (m === 0) return mmSvg("close-circle-outline", size);
    return "";
  }

  function mmIcon(m, size) {
    return `<span class="mm-icon-inline">${mmSvgForLevel(m, size || 16)}</span>`;
  }
  ...
  function sessionTime(session) {
    return Number(session && (session.completed_at || session.ts || session.started_at)) || 0;
  }

  function isCompletedQuizSession(session) {
  ...
  function quizSessionsForLoc(loc, quizStore) {
    const store = quizStore || readQuizSessions();
    const target = normComparable(loc);
    const out = [];
    Object.keys(store || {}).forEach((key) => {
      const arr = Array.isArray(store[key]) ? store[key] : [];
      const keyNorm = normComparable(key);
      arr.forEach((session) => {
        const sid = normComparable(session && (session.concept_id || key));
        if (keyNorm === target || sid === target) out.push(session);
      });
    });
    return out.filter(isCompletedQuizSession).sort((a, b) => sessionTime(b) - sessionTime(a));
  }

  function allQuizSessionsFlat(quizStore) {
    const store = quizStore || readQuizSessions();
    const out = [];
    Object.keys(store || {}).forEach((key) => {
      const arr = Array.isArray(store[key]) ? store[key] : [];
      arr.forEach((session) => {
        if (isCompletedQuizSession(session)) out.push({ key, session });
      });
    });
    return out;
  }

  function directRatingCount(rec) {
    const hist = Array.isArray(rec && rec.history) ? rec.history : [];
    return hist.filter((h) => historyKind(h) === "mastery" && !isAiSource(h && h.source)).length;
  }

  function aiMasteryAcceptCount(rec) {
    const hist = Array.isArray(rec && rec.history) ? rec.history : [];
    return hist.filter((h) => historyKind(h) === "mastery" && isAiSource(h && h.source)).length;
  }

  function quizScore(session) {
    const questions = Array.isArray(session && session.questions) ? session.questions : [];
    const total = questions.length || Number(session && session.total_count) || 0;
    const correct = Number(session && session.correct_count);
    const cc = Number.isFinite(correct) ? correct : questions.filter((q) => q && q.correct).length;
    return { correct: cc, total };
  }

  function latestQuizSummary(sessions) {
    if (!sessions || !sessions.length) return "No AI checks";
    const s = sessions[0];
    const score = quizScore(s);
    const level = mLabel(Number(s.suggested_mastery));
    const when = fmtShortDate(sessionTime(s));
    return `${score.correct}/${score.total || "?"} correct · ${level}${when ? ` · ${when}` : ""}`;
  }

  function lectureInfoFromTags(tagsLike) {
    const tags = Array.isArray(tagsLike)
      ? tagsLike
      : (tagsLike && typeof tagsLike.forEach === "function") ? Array.from(tagsLike) : [];
    ...
    for (const raw of tags) {
      ...
      if (m) {
        ...
        const courseName = COURSE_TAG_MAP[courseCode] || "";
        ...
        return {
          ...
          unitNum: lectureNum,
          lectureNum,
          unitLabel: `${unitNounFromType(unitType)} ${lectureNum}`,
          lectureLabel: `${unitNounFromType(unitType)} ${lectureNum}`
        };
      }
      m = t.match(bare);
      if (m) {
  ...
  async function loadLectureMapOnce() {
    if (mmLectureMap) return mmLectureMap;
    if (mmLectureMapPromise) return mmLectureMapPromise;
    const root = getSiteRootUrl();
    const candidates = [
      new URL("search/search_index.json", root).href,
      new URL("search_index.json", root).href,
    ];
    mmLectureMapPromise = (async () => {
      for (const url of candidates) {
        try {
          ...
          for (const doc of docs) {
            const key = normId(doc && doc.location);
            if (!key) continue;
            const info = lectureInfoFromTags(doc && doc.tags);
            if (!info) continue;
            map.set(normComparable(key), info.unitLabel || `Lecture ${info.lectureNum}`);
          }
          mmLectureMap = map;
          return map;
        } catch (_) {}
      }
      mmLectureMap = new Map();
      return mmLectureMap;
    })();
    return mmLectureMapPromise;
  }

  function getLectureLabelForLoc(loc) {
    const key = normComparable(loc);
    if (mmLectureMap && mmLectureMap.has(key)) return mmLectureMap.get(key) || "";
    return deriveLectureLabel(loc);
  }

  async function loadAiBankOnce() {
    if (mmAiBank) return mmAiBank;
    if (mmAiBankPromise) return mmAiBankPromise;
    const root = getSiteRootUrl();
    const url = new URL("assets/ai-mcq-bank.json", root).href;
    mmAiBankPromise = fetch(url, { credentials: "same-origin", cache: "no-store" })
      .then((r) => r && r.ok ? r.json() : null)
      .then((j) => { mmAiBank = j || { by_concept: {} }; return mmAiBank; })
      .catch(() => { mmAiBank = { by_concept: {} }; return mmAiBank; });
    return mmAiBankPromise;
  }

  function findBankConcept(conceptId) {
    const by = mmAiBank && mmAiBank.by_concept && typeof mmAiBank.by_concept === "object" ? mmAiBank.by_concept : {};
    const target = normComparable(conceptId);
    if (by[conceptId]) return by[conceptId];
    for (const key of Object.keys(by)) {
      if (normComparable(key) === target) return by[key];
    }
    return null;
  }

  function findBankQuestion(conceptId, qid) {
    const concept = findBankConcept(conceptId);
    const qs = Array.isArray(concept && concept.questions) ? concept.questions : [];
    return qs.find((q) => String(q && q.id) === String(qid)) || null;
  }

  function getMetaForLoc(loc, rec, sessions) {
    const rr = normaliseRecord(rec);
    const fromSession = Array.isArray(sessions) && sessions.length ? sessions[0] : null;
    return {
      title: rr.title || cleanTitle(fromSession && (fromSession.concept_title || fromSession.title) || loc.split("/").pop() || loc),
      course: rr.course || prettyCourseName(deriveCourseName(loc)),
      coursePath: rr.coursePath || String(loc || "").split("/").slice(0, -1).join("/"),
    };
  }

  function getConceptKeys(all, quizStore) {
    const keys = new Map();
    Object.keys(all || {}).forEach((k) => { const n = normId(k); if (n) keys.set(normComparable(n), n); });
    Object.keys(quizStore || {}).forEach((k) => { const n = normId(k); if (n) keys.set(normComparable(n), n); });
    Object.keys(quizStore || {}).forEach((k) => {
      const arr = Array.isArray(quizStore[k]) ? quizStore[k] : [];
      arr.forEach((s) => {
        const n = normId(s && s.concept_id || k);
        if (n) keys.set(normComparable(n), n);
      });
    });
    return Array.from(keys.values());
  }

  function globalStats(all, quizStore) {
    let total = 0, m3 = 0, m2 = 0, m1 = 0, m0 = 0, unrated = 0;
    let direct = 0, aiAccepted = 0, views = 0;
    Object.keys(all || {}).forEach((k) => {
      total += 1;
      const rec = normaliseRecord(all[k]);
      if (rec.m === 3) m3 += 1;
      else if (rec.m === 2) m2 += 1;
      else if (rec.m === 1) m1 += 1;
      else if (rec.m === 0) m0 += 1;
      else unrated += 1;
      direct += directRatingCount(rec);
      aiAccepted += aiMasteryAcceptCount(rec);
      views += Number(rec.viewCount) || 0;
    });
    const quizzes = allQuizSessionsFlat(quizStore);
    let correct = 0, attempted = 0;
    quizzes.forEach(({ session }) => {
      const sc = quizScore(session);
      correct += sc.correct;
      attempted += sc.total;
    });
    return { total, m3, m2, m1, m0, unrated, direct, aiAccepted, views, aiChecks: quizzes.length, correct, attempted };
  }

  function buildItems() {
    const all = readAll();
    const quizStore = readQuizSessions();
    const keys = getConceptKeys(all, quizStore);
    const items = keys.map((loc) => {
      const raw = all[loc] || all[Object.keys(all).find((k) => normComparable(k) === normComparable(loc))] || {};
      const rec = normaliseRecord(raw);
      const sessions = quizSessionsForLoc(loc, quizStore);
      const meta = getMetaForLoc(loc, raw, sessions);
      const latestQuiz = sessions.length ? sessionTime(sessions[0]) : 0;
      const ratingRecent = Array.isArray(rec.history)
        ? rec.history.filter((h) => historyKind(h) === "mastery").reduce((mx, h) => Math.max(mx, historyTs(h)), 0)
        : 0;
      const recent = Math.max(ratingRecent, Number(rec.lastReviewed) || 0, Number(rec.lastViewed) || 0, latestQuiz);
      return {
        loc,
        rec,
        sessions,
        title: cleanTitle(meta.title || loc),
        courseName: prettyCourseName(meta.course || deriveCourseName(loc)),
        lectureLabel: getLectureLabelForLoc(loc),
        m: (typeof rec.m === "number") ? rec.m : null,
        views: Number(rec.viewCount) || 0,
        direct: directRatingCount(rec),
        aiAccepted: aiMasteryAcceptCount(rec),
        aiChecks: sessions.length,
        latestQuiz,
        ratingRecent,
        recent,
      };
  ...
  function sortItems(items) {
    ...
    arr.sort((a, b) => {
      ...
      const am = typeof a.m === "number" ? a.m : -1;
      const bm = typeof b.m === "number" ? b.m : -1;
      if (mmSortKey === "mastery") return (bm - am) || (b.recent - a.recent) || at.localeCompare(bt);
      if (mmSortKey === "views") return (b.views - a.views) || (b.recent - a.recent) || at.localeCompare(bt);
      if (mmSortKey === "ai") return (b.aiChecks - a.aiChecks) || (b.latestQuiz - a.latestQuiz) || at.localeCompare(bt);
      if (mmSortKey === "direct") return (b.direct - a.direct) || (b.recent - a.recent) || at.localeCompare(bt);
      if (mmSortKey === "title") return at.localeCompare(bt);
      const aRated = typeof a.m === "number" ? 1 : 0;
      const bRated = typeof b.m === "number" ? 1 : 0;
      return (bRated - aRated) || ((Number(b.ratingRecent) || 0) - (Number(a.ratingRecent) || 0)) || (b.recent - a.recent) || at.localeCompare(bt);
    });
    return arr;
  }

  function filterItems(items) {
    const q = String(mmSearchQuery || "").toLowerCase().trim();
    if (!q) return items;
    const toks = q.split(/\s+/).filter(Boolean);
    return items.filter((item) => {
      const hay = [item.title, item.courseName, item.lectureLabel, item.loc, mLabel(item.m)].join(" ").toLowerCase();
      return toks.every((t) => hay.includes(t));
    });
  }

  function historyLabel(h) {
    if (historyKind(h) === "view") return "Viewed";
    return mLabel((h && Object.prototype.hasOwnProperty.call(h, "m")) ? Number(h.m) : null);
  }

  function historyRowsHtml(rec) {
    const hist = Array.isArray(rec.history) ? rec.history.slice().sort((a, b) => historyTs(b) - historyTs(a)) : [];
    if (!hist.length) return `<div class="mm-empty">No mastery history yet.</div>`;
    return hist.map((h) => {
      const kind = historyKind(h);
      const level = kind === "view" ? null : Number(h && h.m);
      const src = String(h && h.source || "");
      return `
  ...
  function optionTextFromSession(item, index) {
    const opts = Array.isArray(item && item.shown_options) ? item.shown_options : [];
    const text = opts[index];
    return text == null ? "" : String(text);
  }

  function wrongQuestionsHtml(session) {
    const qs = Array.isArray(session && session.questions) ? session.questions : [];
    const wrong = qs.filter((q) => q && q.correct === false);
    if (!wrong.length) return `<div class="mm-good-news">No wrong answers in this check.</div>`;

    return wrong.map((item, idx) => {
      const bankQ = findBankQuestion(session.concept_id, item.qid);
      const questionText = item.question || (bankQ && bankQ.question) || `Question ${escapeHtml(item.qid || idx + 1)}`;
      const explanation = item.explanation || (bankQ && bankQ.explanation) || "";
      const selected = Number(item.selected_index);
      const correct = Number(item.correct_index);
      const yourAnswer = optionTextFromSession(item, selected);
      const correctAnswer = optionTextFromSession(item, correct);
      return `
  ...
  function quizSessionsHtml(loc, sessions) {
    if (!sessions || !sessions.length) return `<div class="mm-empty">No AI concept checks for this concept yet.</div>`;
    return sessions.map((session, idx) => {
      const score = quizScore(session);
      const level = mLabel(Number(session.suggested_mastery));
      const when = fmtTime(sessionTime(session));
      const wrongCount = (Array.isArray(session.questions) ? session.questions : []).filter((q) => q && q.correct === false).length;
      return `
  ...
  function compactMetric(label, value, extraClass) {
    return `<span class="mm-compact-metric ${extraClass || ""}"><strong>${escapeHtml(String(value))}</strong> ${escapeHtml(label)}</span>`;
  }

  function cardHtml(item) {
    const href = hrefForLoc(item.loc);
    const histCount = Array.isArray(item.rec.history) ? item.rec.history.length : 0;
    const statusClass = mClass(item.m);
    return `
  ...
  function renderStatCards(s) {
    const acc = s.attempted ? `${Math.round((s.correct / s.attempted) * 100)}%` : "-";
    return `
  ...
  function ensureManagerStyles() {
    ["mm-manager-style-v5-click-layer", "mm-manager-style-v6", "mm-manager-style-v7", "mm-manager-style-v8-learning-records", "mm-manager-style-v9-compact-inline", "mm-manager-style-v10-lazy-history-ai", "mm-manager-style-v12-mobile-scroll-compact", "mm-manager-style-v13-mobile-safe-compact-actions", "mm-manager-style-v14-mobile-panel-floor", "mm-manager-style-v14-mobile-safe-continuation", "mm-manager-style-v15-mobile-doc-surface"].forEach((id) => {
      try {
    ...
    try {
      const legacyFloor = document.getElementById("mm-mobile-panel-floor");
      if (legacyFloor && legacyFloor.parentNode) legacyFloor.parentNode.removeChild(legacyFloor);
    } catch (_) {}
  ...
  function exportData() {
    const payload = {
      type: "learning_records_export",
      version: 1,
      exportedAt: Date.now(),
      mastery: readAll(),
      aiQuizSessions: readQuizSessions(),
    };
    const d = new Date();
    const pad = (n) => String(n).padStart(2, "0");
    const stamp = `${d.getFullYear()}${pad(d.getMonth() + 1)}${pad(d.getDate())}-${pad(d.getHours())}${pad(d.getMinutes())}${pad(d.getSeconds())}`;
    downloadText(`learning-records-${stamp}.json`, JSON.stringify(payload, null, 2));
  }
  ...
  function importDataFromFile(file) {
    ...
    reader.onload = () => {
      try {
        ...
        if (!confirm("Import this file and replace current mastery and AI quiz data where included?")) return;
        if (parsed && parsed.type === "learning_records_export") {
          if (parsed.mastery && typeof parsed.mastery === "object") writeAll(parsed.mastery);
          if (parsed.aiQuizSessions && typeof parsed.aiQuizSessions === "object") writeQuizSessions(parsed.aiQuizSessions);
        } else {
          const data = parsed && parsed.data && typeof parsed.data === "object" ? parsed.data : parsed;
          if (!data || typeof data !== "object" || Array.isArray(data)) throw new Error("Invalid import file");
          writeAll(data);
        }
        if (typeof mmRender === "function") mmRender();
      } catch (_) {
        alert("Import failed. Please check that the file is a valid JSON export.");
      }
    };
  ...
  function setLevelFromManager(loc, value) {
    const all = readAll();
    const actualKey = Object.keys(all).find((k) => normComparable(k) === normComparable(loc)) || loc;
    const raw = (all[actualKey] && typeof all[actualKey] === "object") ? all[actualKey] : {};
    const rec = normaliseRecord(raw);
    const meta = getMetaForLoc(actualKey, raw, quizSessionsForLoc(actualKey));

    if (window.ConceptMastery && typeof window.ConceptMastery.setLevel === "function") {
      window.ConceptMastery.setLevel(actualKey, value === "" ? null : Number(value), meta, { source: "manager" });
      return;
    }

    if (value === "") {
      const viewHistory = (Array.isArray(rec.history) ? rec.history : []).filter((h) => historyKind(h) === "view");
      raw.m = null;
      raw.unrated = true;
      raw.state = "unrated";
      raw.lastReviewed = 0;
      raw.reviewCount = 0;
      raw.counts = { full: 0, know: 0, fuzzy: 0, dont: 0 };
      raw.history = viewHistory;
    } else {
      const ts = Date.now();
      raw.m = Number(value);
      delete raw.unrated;
      if (raw.state === "unrated") delete raw.state;
      raw.lastReviewed = ts;
      raw.visited = true;
      raw.history = Array.isArray(raw.history) ? raw.history.slice() : [];
      raw.history.push({ kind: "mastery", m: raw.m, ts, source: "manager" });
      raw.reviewCount = (Number(raw.reviewCount) || 0) + 1;
    }
    raw.title = meta.title || raw.title || "";
    raw.course = meta.course || raw.course || "";
    raw.coursePath = meta.coursePath || raw.coursePath || "";
    all[actualKey] = raw;
    try {
      if (window.MkAccountData && typeof window.MkAccountData.recordMastery === "function") {
        window.MkAccountData.recordMastery(actualKey, value === "" ? null : Number(value), { path: actualKey, title: raw.title || meta.title || "", level: value === "" ? null : Number(value), mastery: value === "" ? null : Number(value), source: "mastery-manager", changeKind: value === "" ? "clear" : "change" }, { source: "mastery-manager" });
      }
    } catch (_) {}
    writeAll(all);
  }

  function deleteConceptRecord(loc) {
    const all = readAll();
    const key = Object.keys(all).find((k) => normComparable(k) === normComparable(loc));
    if (key) {
      try {
        if (window.MkAccountData && typeof window.MkAccountData.recordMastery === "function") {
          window.MkAccountData.recordMastery(key, null, { path: key, level: null, mastery: null, source: "mastery-manager-delete", changeKind: "clear", xpEligible: false }, { source: "mastery-manager-delete" });
        }
      } catch (_) {}
      delete all[key];
    }
  ...
  function rawRecordForLoc(loc) {
    const all = readAll();
    const key = Object.keys(all || {}).find((k) => normComparable(k) === normComparable(loc)) || loc;
    return (all && all[key] && typeof all[key] === "object") ? all[key] : {};
  }

  function closeOpenCardPanels(root, exceptCard) {
    const scope = root || document;
    scope.querySelectorAll(".mm-card").forEach((card) => {
      if (exceptCard && card === exceptCard) return;
      card.classList.remove("is-popover-open");
      card.querySelectorAll(".mm-history-menu").forEach((menu) => menu.setAttribute("hidden", ""));
      card.querySelectorAll(".mm-expand-row").forEach((row) => row.setAttribute("hidden", ""));
      card.querySelectorAll(".mm-panel-toggle").forEach((btn) => {
  ...
  function renderHistoryMenu(card) {
    if (!card) return;
    const loc = card.getAttribute("data-loc") || "";
    const menu = card.querySelector(".mm-history-menu");
    const target = menu && menu.querySelector('[data-lazy-panel="history"]');
    if (!target) return;
    if (target.getAttribute("data-rendered") === "1") return;
    const rec = normaliseRecord(rawRecordForLoc(loc));
    target.innerHTML = historyRowsHtml(rec);
    target.setAttribute("data-rendered", "1");
    typesetMathIn(target);
  }

  function renderAiDetailsPanel(card) {
    if (!card) return;
    const loc = card.getAttribute("data-loc") || "";
    const row = card.querySelector('[data-panel-body="ai"]');
    const target = row && row.querySelector('[data-lazy-panel="ai"]');
    if (!target) return;
    if (target.getAttribute("data-rendered") === "1") return;

    const sessions = quizSessionsForLoc(loc, readQuizSessions());
    const draw = () => {
      target.innerHTML = quizSessionsHtml(loc, sessions);
      target.setAttribute("data-rendered", "1");
      typesetMathIn(target);
    };

    if (sessions.length && !mmAiBank) {
      target.innerHTML = '<div class="mm-empty">Loading AI check details…</div>';
      loadAiBankOnce().then(draw).catch(draw);
    } else {
      draw();
    }
  }

  function mmIsTouchLikeViewport() {
    try {
  ...
  function mmClamp(value, min, max) {
    return Math.min(max, Math.max(min, value));
  }

  function mmIsIOSWebKitMobile() {
    try {
  ...
  function mmReadSafeAreaBottomInsetPx() {
    try {
      let probe = document.getElementById("mm-safe-area-probe");
      if (!probe) {
        probe = document.createElement("div");
        probe.id = "mm-safe-area-probe";
  ...
  function mmIOSCompleteToolbarOcclusionPx() {
    if (!mmIsTouchLikeViewport() || !mmIsIOSWebKitMobile()) return 0;
    try {
      ...
      const safe = Math.max(0, mmReadSafeAreaBottomInsetPx());
      const screenGap = screenH > 0 ? Math.max(0, Math.round(screenH - layoutH - safe)) : 0;
      const raw = Math.max(visualGap, screenGap);
      if (raw < 56) return 0;
      return mmClamp(raw, 64, 260);
    } catch (_) { return 0; }
  }

  function mmRemoveLegacyMobilePanelFloor() {
    try {
      const floor = document.getElementById("mm-mobile-panel-floor");
      if (floor && floor.parentNode) floor.parentNode.removeChild(floor);
    } catch (_) {}
  }
  ...
  function mmUpdateViewportMetrics() {
    ...
    try {
      ...
      if (vvH > 0) modal.style.setProperty("--mm-vh", mmPx(vvH));

      if (!mmIsTouchLikeViewport()) {
        modal.style.setProperty("--mm-mobile-top-pad", "0px");
        modal.style.setProperty("--mm-mobile-bottom-pad", "0px");
        modal.style.removeProperty("--mm-doc-left");
        modal.style.removeProperty("--mm-doc-top");
        modal.style.removeProperty("--mm-doc-width");
        modal.style.removeProperty("--mm-doc-height");
        modal.style.removeProperty("--mm-visible-height");
        modal.style.removeProperty("--mm-ios-hidden-tail");
        const panel = document.getElementById("mm-panel");
        if (panel) panel.classList.remove("mm-ios-bottom-continued");
        return;
      }

      // Document-layer geometry, copied from the sidebar idea: the visual panel
      // is not only as tall as the visible viewport. It continues into the area
      // sampled by iOS Safari's bottom toolbar.
      const safeStrip = Math.max(mmReadSafeAreaBottomInsetPx(), vv ? Math.max(0, Math.round(layoutH - vvBottom)) : 0, mmIOSCompleteToolbarOcclusionPx());
      const visibleBottom = vv ? Math.max(0, vvBottom) : layoutH;
      const layoutBottom = Math.max(layoutH, visibleBottom) + Math.max(0, safeStrip);
      const docLeft = mmPageScrollXNow() + vvLeft;
      const docTop = mmPageScrollYNow() + vvTop;
      ...
      modal.style.setProperty("--mm-doc-left", mmPx(docLeft));
      modal.style.setProperty("--mm-doc-top", mmPx(docTop));
      modal.style.setProperty("--mm-doc-width", mmPx(vvW || layoutW));
      modal.style.setProperty("--mm-doc-height", mmPx(docHeight));
      modal.style.setProperty("--mm-visible-height", mmPx(visibleHeight));
      modal.style.setProperty("--mm-ios-hidden-tail", mmPx(hiddenTail));
      modal.style.setProperty("--mm-mobile-top-pad", "0px");
      modal.style.setProperty("--mm-mobile-bottom-pad", "0px");

      const panel = document.getElementById("mm-panel");
      if (panel) panel.classList.toggle("mm-ios-bottom-continued", hiddenTail > 12 || safeStrip > 12);
    } catch (_) {}
  }

  function mmBindViewportMetricsOnce() {
    if (window.__mmViewportMetricsBoundV16) return;
    window.__mmViewportMetricsBoundV16 = true;
    const update = () => mmUpdateViewportMetrics();
  ...
  function closeModal() {
    mmRemoveLegacyMobilePanelFloor();
  ...
  function ensureModal() {
    mmRemoveLegacyMobilePanelFloor();
    ensureManagerStyles();
    mmBindViewportMetricsOnce();
    const existing = document.getElementById("mm-modal");
    if (existing) {
      if (existing.getAttribute("data-mm-manager-build") === "v18") return;
      try { existing.remove(); } catch (_) {}
    }

    const wrap = document.createElement("div");
    wrap.id = "mm-modal";
    wrap.setAttribute("data-mm-manager-build", "v18");
    wrap.innerHTML = `
    ...
    // Keep all touch/click activity inside the modal layer on mobile.
    ...
    mmRender = function render() {
      const all = readAll();
      const quizStore = readQuizSessions();
      const s = globalStats(all, quizStore);
      const statEl = document.getElementById("mm-stat");
      if (statEl) statEl.innerHTML = renderStatCards(s);
      ...
      list.innerHTML = items.length
        ...
        : `<div class="mm-empty" style="padding:16px">No records match this filter.</div>`;
      typesetMathIn(list);

      list.querySelectorAll(".mm-level").forEach((sel) => {
        sel.addEventListener("change", () => {
          setLevelFromManager(sel.getAttribute("data-loc") || "", sel.value);
          if (typeof mmRender === "function") mmRender();
        });
      });
      list.querySelectorAll(".mm-del").forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          if (!confirm("Delete this concept mastery record? AI quiz sessions for this concept will be kept.")) return;
          deleteConceptRecord(loc);
          if (typeof mmRender === "function") mmRender();
        });
      });

      list.querySelectorAll(".mm-panel-toggle").forEach((btn) => {
        btn.addEventListener("click", (event) => {
          ...
          if (panel === "history") {
            ...
            closeOpenCardPanels(list, card);
            card.classList.remove("is-popover-open");
            card.querySelectorAll(".mm-history-menu").forEach((m) => m.setAttribute("hidden", ""));
            card.querySelectorAll(".mm-expand-row").forEach((row) => row.setAttribute("hidden", ""));
            card.querySelectorAll(".mm-panel-toggle").forEach((b) => {
              b.classList.remove("is-open");
              b.setAttribute("aria-expanded", "false");
            });

            if (willOpen) {
              renderHistoryMenu(card);
          ...
          if (panel === "ai") {
            const body = card.querySelector('[data-panel-body="ai"]');
            if (!body) return;
      ...
      const sortEl = document.getElementById("mm-sort");
      if (sortEl) sortEl.value = mmSortKey;
      const filterEl = document.getElementById("mm-filter");
      if (filterEl && filterEl.value !== mmSearchQuery) filterEl.value = mmSearchQuery;
    };

    wrap.addEventListener("click", (e) => {
      const panel = document.getElementById("mm-panel");
      if (e.target === wrap || (panel && e.target && !panel.contains(e.target))) {
        e.preventDefault();
        closeModal();
        return;
      }
      if (panel && e.target && panel.contains(e.target) && !e.target.closest(".mm-inline-actions") && !e.target.closest(".mm-expand-row")) {
        closeOpenCardPanels(panel, null);
      }
    });

    document.getElementById("mm-close").addEventListener("click", closeModal);
    document.getElementById("mm-sort").addEventListener("change", (e) => {
      mmSortKey = e.target.value || "recent";
      if (typeof mmRender === "function") mmRender();
    });
    document.getElementById("mm-filter").addEventListener("input", (e) => {
      mmSearchQuery = e.target.value || "";
      if (typeof mmRender === "function") mmRender();
    });
    document.getElementById("mm-clear").addEventListener("click", () => {
      if (!confirm("Clear all mastery records and AI quiz sessions stored in this browser?")) return;
      try { if (window.MkAccountData && typeof window.MkAccountData.clearMasteryAndQuiz === "function") window.MkAccountData.clearMasteryAndQuiz(); } catch (_) {}
    ...
    if (!window.__mmCloudStorageEventsBoundV2) {
      window.__mmCloudStorageEventsBoundV2 = true;
      const refreshOpenManager = () => {
        const modal = document.getElementById("mm-modal");
        if (!modal || modal.style.display !== "block") return;
        if (typeof mmRender === "function") mmRender();
      };
      window.addEventListener("conceptMasteryChanged", refreshOpenManager);
      window.addEventListener("mk-ai-quiz-sessions-changed", refreshOpenManager);
      window.addEventListener("mk-local-activity-change", refreshOpenManager);
    }

    loadLectureMapOnce().then(() => { if (typeof mmRender === "function") mmRender(); }).catch(() => {});
    mmRender();
  }

  function syncFromCloudOnOpen(attempt) {
    try {
      const api = window.MkLocalActivity;
      const n = Number(attempt || 0);
      if (!api) {
        if (n < 12) window.setTimeout(() => syncFromCloudOnOpen(n + 1), 250);
        return;
      }
      const profile = typeof api.getProfile === "function" ? api.getProfile() : null;
      if (!profile || !(profile.accountKey || profile.name)) return;

      const rerender = () => { if (typeof mmRender === "function") mmRender(); };
      // Opening the manager must stay a local read.  It may refresh the small
      // cloud metadata/count used by the account panel, but it must not download
      // account chunks or start a merge/upload cycle.
      if (window.MkAccountData && typeof window.MkAccountData.refreshCloudStatus === "function") {
        const p = window.MkAccountData.refreshCloudStatus({
          reason: "mastery-manager-open-metadata",
          timeoutMs: 30000,
          writeDisplaySummary: false,
          lightStatus: true,
          cloudCountOnly: true
        });
        if (p && typeof p.then === "function") p.then(rerender).catch(() => {});
      }
    } catch (_) {}
  }
  ...
  function open() {
    ...
    const m = document.getElementById("mm-modal");
    if (!m) return;
    mmUpdateViewportMetrics();
    m.style.display = "block";
    try { document.documentElement.classList.add("mm-modal-open"); } catch (_) {}
    try { document.body.classList.add("mm-modal-open"); } catch (_) {}
    window.setTimeout(mmUpdateViewportMetrics, 60);
    if (typeof mmRender === "function") mmRender();
    window.setTimeout(syncFromCloudOnOpen, 80);
  }
