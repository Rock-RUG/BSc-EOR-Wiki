(function () {
  ...
  const STYLE_ID = "mk-current-course-bar-style-v10-seam-close";
...
`;
  ...
  function toAbsoluteSiteHref(rel) {
  ...
  function findScopeHref(scopeNode, yearSeg, courseSeg) {
    ...
    if (directAnchor) return directAnchor.href || directAnchor.getAttribute("href") || "";

    const want = [];
    if (yearSeg && courseSeg) {
      want.push(`${yearSeg}/${courseSeg}/`, `${yearSeg}/${courseSeg}/index.html`);
    } else if (yearSeg) {
      want.push(`${yearSeg}/`, `${yearSeg}/index.html`);
    }

    if (want.length) {
      const links = Array.from(scopeNode.querySelectorAll('a.md-nav__link[href]'));
      for (const a of links) {
        const rel = normaliseHrefToRel(a.getAttribute("href"));
        if (want.includes(rel)) return a.href || a.getAttribute("href") || "";
      }
    }

    if (yearSeg && courseSeg) return toAbsoluteSiteHref(`${yearSeg}/${courseSeg}/`);
    if (yearSeg) return toAbsoluteSiteHref(`${yearSeg}/`);
    return "";
  }
  ...
  function measuredBarHeight(bar) {
  ...
  function syncSiblingStickyOffsets() {
    const bar = document.getElementById(BAR_ID);
    if (!bar) return;
    const dock = document.getElementById("mk-sidebar-sortdock");
    const h = measuredBarHeight(bar);
    const scrollWrap = bar.closest(".md-sidebar__scrollwrap") || bar.closest(".md-sidebar__inner") || bar.parentElement;
    if (scrollWrap && scrollWrap.style) {
      scrollWrap.style.setProperty("--msb-current-bar-h", `${h}px`);
    }
    if (dock) {
      dock.style.top = isMobileViewport() ? `${Math.max(44, h - 6)}px` : `${Math.max(0, h - 2)}px`;
    }
  }

  function emitLayoutChangedSettled() {
    syncSiblingStickyOffsets();
    emitLayoutChanged();
  ...
  function apply() {
    ...
    const titleLink = bar.querySelector(".ccb-titlelink");
    ...
    const titleEl = directNavLink(scopeNode) || scopeNode.querySelector(".md-nav__link");
    const titleText = cleanTitleText(titleEl ? titleEl.textContent : "");
    if (!titleText) {
      bar.style.display = "none";
      emitLayoutChangedSettled();
      return;
    ...
    const scopeHref = findScopeHref(scopeNode, year, course);

    titleSpan.textContent = titleText;
    if (titleLink) {
      const href = scopeHref || "#top";
      titleLink.href = href;
      titleLink.title = course ? `Open ${titleText} index` : `Open ${titleText}`;
    ...
    const sidebarApi = course && window.MkSidebarNavSort &&
      typeof window.MkSidebarNavSort.isCurrentCourseOpen === "function" &&
      typeof window.MkSidebarNavSort.toggleCurrentCourse === "function"
    ...
    if (!showArrow || !toggleBtn || !iconSpan || (!sidebarApi && !toggle)) {
    ...
    const syncIcon = () => {
      const open = sidebarApi ? !!sidebarApi.isCurrentCourseOpen() : !!(toggle && toggle.checked);
      iconSpan.classList.toggle("is-open", open);
    ...
    if (!sidebarApi && toggle && !toggleListenerBound.has(toggle)) {
    ...
    toggleBtn.onclick = (e) => {
      ...
      e.stopPropagation();
      if (sidebarApi) {
        sidebarApi.toggleCurrentCourse();
      } else if (toggle) {
        toggle.click();
      }
  ...
  window.addEventListener("mk:sidebar-sort-rendered", scheduleApply, { passive: true });
  window.addEventListener("mk:sidebar-current-course-toggle", scheduleApply, { passive: true });
