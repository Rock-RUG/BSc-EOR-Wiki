...
function collectYearCourseItems(yearLink,root,linkBase){if(!yearLink)return[];let yearUrl;try{yearUrl=new URL(hrefOf(yearLink),document.baseURI);}catch(_){return[];}
const basePath=canonicalDirPath(yearUrl.pathname);const seen=new Map();const source=root||document;const base=linkBase||document.baseURI;const selectors=[".md-sidebar--primary .md-nav a.md-nav__link[href]",".md-nav--primary a.md-nav__link[href]",".md-nav a.md-nav__link[href]",".md-tabs__list a.md-tabs__link[href]"];$all(selectors.join(","),source).forEach((a)=>{let u;try{u=new URL(a.getAttribute("href")||hrefOf(a),base);}catch(_){return;}
...
const yearCourseRequests=new Map();function loadYearCourseItems(item){const href=item&&item.href;const yearUrl=safeUrl(href);if(!yearUrl||yearUrl.origin!==window.location.origin)return Promise.resolve([]);const key=canonicalDirPath(yearUrl.pathname);if(yearCourseRequests.has(key))return yearCourseRequests.get(key);const request=fetch(yearUrl.toString(),{credentials:"same-origin"}).then((response)=>response.ok?response.text():"").then((html)=>html?collectYearCourseItems(item,new DOMParser().parseFromString(html,"text/html"),yearUrl.toString()):[]).catch(()=>[]).then((courses)=>{if(!courses.length)yearCourseRequests.delete(key);return courses;});yearCourseRequests.set(key,request);return request;}
...
const needsDirectory=!(item&&item.children&&item.children.length);fillMobileYearCoursePanel(panel,item||{},items||[]);if(state.openPanel===panel&&state.openTrigger)placePanel(panel,state.openTrigger);if(keyboardActivation){const first=visibleMenuItems(panel)[0];if(first)first.focus({preventScroll:true});}
if(needsDirectory)loadYearCourseItems(item).then((courses)=>{if(!courses.length)return;item.children=courses.concat({label:`Browse all ${item.label} courses`,href:item.href});if(state.openPanel!==panel||panel.__mkRtYearItem!==item)return;const hadFocus=panel.contains(document.activeElement);fillMobileYearCoursePanel(panel,item,items||[]);if(state.openTrigger)placePanel(panel,state.openTrigger);if(hadFocus){const first=visibleMenuItems(panel)[0];if(first)first.focus({preventScroll:true});}});};["pointerdown","mousedown","touchstart"].forEach((eventName)=>{arrow.addEventListener(eventName,stopOnly,{capture:true,passive:false});});arrow.addEventListener("touchend",openCoursesFromArrow,{capture:true,passive:false});arrow.addEventListener("pointerup",openCoursesFromArrow,true);arrow.addEventListener("click",openCoursesFromArrow,true);arrow.addEventListener("keydown",(ev)=>{if(ev.key==="Enter"||ev.key===" ")openCoursesFromArrow(ev);},true);row.appendChild(arrow);panel.appendChild(row);});}
function fillMobileYearCoursePanel(panel,item,parentItems){if(!panel)return;resetPanelInteractionState(panel);panel.classList.add("mk-rt-mobile-year-panel","mk-rt-mobile-year-course-panel");panel.classList.remove("mk-rt-mobile-trending-panel");panel.__mkRtYearItem=item;panel.innerHTML="";const back=document.createElement("button");back.type="button";back.className="mk-rt-panel-item mk-rt-year-back";back.textContent=`‹ Back to Year`;back.setAttribute("role","menuitem");back.addEventListener("click",(ev)=>{const keyboardActivation=ev.detail===0;ev.preventDefault();ev.stopPropagation();resetPanelInteractionState(panel);try{panel.__mkRtParentRect=null;}catch(_){}
fillMobileYearPanel(panel,parentItems||[]);if(state.openPanel===panel&&state.openTrigger)placePanel(panel,state.openTrigger);if(keyboardActivation){const arrows=panel.querySelectorAll(".mk-rt-year-arrow");const previous=arrows[Math.max(0,(parentItems||[]).indexOf(item))];if(previous)previous.focus({preventScroll:true});}});panel.appendChild(back);const courses=item&&item.children&&item.children.length?item.children:item&&item.href?[{label:`Browse all ${item.label} courses`,href:item.href}]:[];if(!courses.length){panel.appendChild(el("div","mk-rt-sep"));const empty=document.createElement("button");empty.type="button";empty.className="mk-rt-panel-item mk-rt-year-empty";empty.textContent="No course list found";empty.setAttribute("role","menuitem");empty.disabled=true;panel.appendChild(empty);return;}
